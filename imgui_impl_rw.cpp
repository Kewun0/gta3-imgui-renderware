...
void ImGui_ImplRenderWare_RenderDrawData(ImDrawData* draw_data)
{
	...
	const RwReal nearScreenZ = RwIm2DGetNearScreenZ();	// CSprite2d::NearScreenZ 009DAA60
	const RwReal recipNearClip = CSprite2d::RecipNearClip;	// CSprite2d::RecipNearClip 009DAA64
	...
	if (!g_pVB || g_VertexBufferSize < draw_data->TotalVtxCount)
	{
		...
		if (!g_pVB)
		{
			printf("GUI | Error: couldn't allocate vertex buffer (size: %d)", g_VertexBufferSize);
			return;
		}
		printf("GUI | Vertex buffer reallocated. Size: %d", g_VertexBufferSize);
	}
...
{
	printf("GUI | CreateDeviceObjects.");
	if (g_FontRaster) printf("GUI | Warning: Font raster != 0");
...
IMGUI_API LRESULT ImGui_ImplRW_WndProcHandler(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
